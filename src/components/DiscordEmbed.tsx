import type { IEmbed, IEmbedField } from "../types";
import Markdown from "../lib/markdown/Markdown";

export default function DiscordEmbed({ embed }: { embed: IEmbed }) {
  const fieldRows: IEmbedField[][] = [];
      ...
      <div className="overflow-hidden p-[.5rem_1rem_1rem_.75rem] inline-grid grid-cols-[auto] grid-rows-[auto]">
        ...
        {embed.title ? (
          <a
            ...
            rel="noopener noreferrer"
            role="button"
            className={`min-w-0 inline-block font-semibold col-[1/1] mt-2 ${
              embed.url ? "hover:underline text-[#00b0f4]" : "cursor-text"
            }`}>
            <Markdown type="header" className="text-current">
              {embed.title}
            </Markdown>
          </a>
        ) : null}
        ...
        {embed.thumbnail ? (
          <div className="min-w-0 row-[1/8] col-[2/2] mt-2 ml-4 shrink-0 justify-self-end block max-w-20 max-h-20 rounded-[3px] cursor-pointer overflow-hidden">
            <img
              className="object-contain max-h-full max-w-full float-right"
